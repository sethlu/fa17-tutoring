...
window.addEventListener("DOMContentLoaded", function () {
    ...
    let container = document.querySelector("#bary-antialiasing.canvas-container");
    ...
    function update() {
        ...
        function getPerpendicularVector(a, b) {
            let ox = controls[a].circle_control_x * container.offsetWidth;
            let oy = controls[a].circle_control_y * container.offsetHeight;

            let dx = controls[b].circle_control_x * container.offsetWidth - ox;
            let dy = controls[b].circle_control_y * container.offsetHeight - oy;
        ...
        let perpendicularVectors = [
            getPerpendicularVector(0, 1),
            getPerpendicularVector(1, 2),
            getPerpendicularVector(2, 0)
        ];
        ...
        for (let x = 5; x < container.offsetWidth; x += 10) {
            for (let y = 5; y < container.offsetHeight; y += 10) {
                ...
                function getColor(x, y) {
                    ...
                    for (let i = 0; i < 3; i++) {
                        let px = x - perpendicularVectors[i].ox;
                        let py = y - perpendicularVectors[i].oy;
                        let rdx = perpendicularVectors[i].rdx;
                        let rdy = perpendicularVectors[i].rdy;
